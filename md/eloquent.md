...
- [Простейшее использование Usage](#basic-usage)
...
Теперь когда вы вызовите метод `delete`, поле `deleted_at` будет установлено в значение текущего времени. При запросе моделей, испльзуюих мягкое удаление, "удалённые" модели не будут включены в результат запроса. Для отображения всех моделей, в том числе удалённых, используйте метод `withTrashed`.
...
Метод `restore` можно использвоать и в отношениях:
...
**Использование собственнго формата времени:**
...
Заготовки позволяют вам повторно использовать логику запросов в моделях. Для создания заготовки просто начните имя метода с `scope`:
...
Для создания обратного отношения в модели `Phone` используйте метод `belongsTo`:
...
При обновлении связей `belongsTo` вы можете использовать метод `associate`. Он установит внешний ключ на связанной модели:
...
По умолчанию Eloquent преобразует поля `created_at`, `updated_at` и `deleted_at` в объекты [Carbon](https://github.com/briannesbitt/Carbon), которые предоставляют множество полезных методов, расширяя стандартный класс PHP `DateTime`s.
