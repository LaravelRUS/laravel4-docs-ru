...
Для начала настроим соединение с БД. Все соединения настраиваются в файле `app/config/database.php`. По умолчанию Laravel использует SQLite, которая находится в папке `app/database`. Вы можете изменить опцию `driver` option на `mysql` и настроить соединение с MySQL в этом конфигурационном файле.
